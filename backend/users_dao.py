...
def create_user(email, password, name, bank_balance, dark, quiet, region):
    """
    ...
    user = User(email=email, password=password, name= name, bank_balance = bank_balance, dark= dark, quiet = quiet, region = region)
    db.session.add(user)
    db.session.commit()
